              ...
              fluidPage(
                sidebarLayout(
                  sidebarPanel(
                    ...
                    box(selectInput(inputId = "nationality",
                                    label = "Player's nationality",
                                    choices = sort(unique(osm_df$nationality)),
                                    multiple = TRUE)
                    ),
                    box(checkboxGroupInput(inputId = "position",
                    ...
                    box(selectInput(inputId = "league",
                                    label = "League",
                                    choices = sort(unique(osm_df$league_name)),
                                    multiple = TRUE)
                    ),
                    box(selectizeInput(inputId = "team",
                                       label = "Team",
                                       choices = NULL,
                                       multiple = TRUE)
                    )
                    ),
                  mainPanel(box(reactableOutput("players_table")))
                  )
                )
      )
    )
